...
import cn.iocoder.yudao.module.admin.dal.mysql.oauth2.OAuthAdmin2ClientMapper;
...
@Validated
@Slf4j
public class OAuthAdmin2ClientServiceImpl implements OAuth2ClientService {

    @Resource
    private OAuthAdmin2ClientMapper oauth2ClientMapper;

    @Override
    public Long createOAuth2Client(OAuth2ClientSaveReqVO createReqVO) {
        validateClientIdExists(null, createReqVO.getClientId());
        // 插入
        AdminOAuth2ClientDO client = BeanUtils.toBean(createReqVO, AdminOAuth2ClientDO.class);
    ...
    public void updateOAuth2Client(OAuth2ClientSaveReqVO updateReqVO) {
        ...
        // 更新
        AdminOAuth2ClientDO updateObj = BeanUtils.toBean(updateReqVO, AdminOAuth2ClientDO.class);
    ...
    void validateClientIdExists(Long id, String clientId) {
        AdminOAuth2ClientDO client = oauth2ClientMapper.selectByClientId(clientId);
        if (client == null) {
            return;
        }
    ...
    @Override
    public AdminOAuth2ClientDO getOAuth2Client(Long id) {
    ...
    @Cacheable(cacheNames = RedisKeyConstants.OAUTH_CLIENT, key = "#clientId",
            unless = "#result == null")
    public AdminOAuth2ClientDO getOAuth2ClientFromCache(String clientId) {
        return oauth2ClientMapper.selectByClientId(clientId);
    }

    @Override
    public PageResult<AdminOAuth2ClientDO> getOAuth2ClientPage(OAuth2ClientPageReqVO pageReqVO) {
        return oauth2ClientMapper.selectPage(pageReqVO);
    }

    @Override
    public AdminOAuth2ClientDO validOAuthClientFromCache(String clientId, String clientSecret, String authorizedGrantType,
                                                    Collection<String> scopes, String redirectUri) {
        // 校验客户端存在、且开启
        AdminOAuth2ClientDO client = getSelf().getOAuth2ClientFromCache(clientId);
        if (client == null) {
            throw exception(OAUTH2_CLIENT_NOT_EXISTS);
        }
     ...
     * @return 自己
     */
    private OAuthAdmin2ClientServiceImpl getSelf() {
