...
echo "上传文件"
# 2. upload local file
scp -l 1000000 -o ConnectTimeout=60000 ./yudao-server/target/yudao-server.jar root@47.236.143.182:/opt/backend/
echo "启动远程服务"
# 3. 关闭远程java进程
ssh root@47.236.143.182 "ps -ef | grep java | grep -v grep | awk '{print \$2}' | xargs kill -9"
# 4. 启动远程java进程
ssh root@47.236.143.182 "cd  /opt/backend/ && nohup ./start.sh> /opt/backend/yudao-server.log 2>&1 &"
echo "deploy success"
set +e